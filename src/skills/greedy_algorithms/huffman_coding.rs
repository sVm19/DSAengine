...
impl Ord for HeapEntry {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        ...
        other.freq.cmp(&self.freq).then(other.node_idx.cmp(&self.node_idx))
    }
}
...
impl HuffmanCoding {
    ...
    pub fn build_codes(frequencies: &[(u8, u64)]) -> Vec<(u8, String)> {
        ...
        for &(sym, freq) in frequencies {
            ...
            heap.push(HeapEntry { freq, node_idx: idx });
            AgentLogger::log(
                AgentFeedback::Step,
                format!("Leaf node: symbol='{}' (0x{sym:02X}), freq={freq}.", sym as char),
            );
        }
        ...
        while heap.len() > 1 {
            ...
            heap.push(HeapEntry { freq: parent_freq, node_idx: parent_idx });
        }
        ...
        while let Some((idx, code)) = stack.pop() {
            ...
            if let Some(sym) = node.symbol {
                // Leaf: emit code.
                let final_code = if code.is_empty() { "0".to_string() } else { code };
                AgentLogger::log(
                    AgentFeedback::Step,
                    format!("Symbol '{}': code=\"{final_code}\" (len={}) freq={}", sym as char, final_code.len(), node.freq),
                );
    ...
    pub fn average_code_length(frequencies: &[(u8, u64)], codes: &[(u8, String)]) -> f64 {
        ...
        if total_freq == 0 { return 0.0; }

        let code_map: std::collections::HashMap<u8, usize> = codes
            .iter()

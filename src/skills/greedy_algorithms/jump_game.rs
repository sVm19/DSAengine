...
impl JumpGame {
    ...
    pub fn can_reach(nums: &[u32]) -> bool {
        ...
        for i in 0..n {
            ...
            if max_reach >= n - 1 {
                AgentLogger::log(
                    ...
                    format!("End reachable: max_reach={max_reach} covers last index {}.", n - 1),
                );
                return true;
            }
        }
        ...
        AgentLogger::log(
            AgentFeedback::Success,
            format!("Jump-game result: {}.", if result { "reachable" } else { "unreachable" }),
        );
        result
    }
...
#[macros::mcp_tool(name = "jump_game", description = "Use this for solving jump game problems. Trigger Keywords: jump_game, jump game, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results.")]

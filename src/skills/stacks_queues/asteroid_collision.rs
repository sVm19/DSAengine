...
impl AsteroidCollision {
    ...
    pub fn solve(asteroids: &[i32]) -> Vec<i32> {
        ...
        AgentLogger::log(
            AgentFeedback::Info,
            format!(
                "Asteroid collision simulation started for {} asteroids.",
                asteroids.len()
            ),
        );
            ...
            while alive && ast < 0 && !stack.is_empty() && *stack.last().unwrap() > 0 {
                ...
                if top < ast_mass {
                    ...
                    AgentLogger::log(
                        AgentFeedback::Step,
                        format!(
                            "Asteroid {ast} and top asteroid {top} mutually destroy each other."
                        ),
                    );
                    stack.pop();
                    alive = false;
...
async fn handle_asteroid_collision(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'asteroids' (array of i32, positive=right, negative=left).".to_string(),
    })?;

    let result = {
        let result = AsteroidCollision::solve(&req.asteroids);
        json!({ "remaining": result })
    };

    let solver = AsteroidCollision;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["simulate"],
    }))
    ...
    .with_description("Asteroid collision simulated."))
}

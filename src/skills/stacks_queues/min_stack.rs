...
impl MinStack {
    ...
    pub fn push(&mut self, val: i32) {
        ...
        AgentLogger::log(AgentFeedback::Step, format!("Pushed {val}; current min is {current_min}."));
    }
...
#[macros::mcp_tool(name = "stacks_queues.min_stack", description = "Use this for solving min stack problems. Trigger Keywords: min_stack, min stack, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results.")]
...
async fn handle_min_stack(payload: Value) -> DsaResult<ResultBox> {
    let req: MinStackRequest = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        message: format!("Invalid MinStackRequest: {e}"),
        hint: "Provide 'operations' with entries like {\"type\":\"push\",\"value\":3}.".to_string(),
    })?;

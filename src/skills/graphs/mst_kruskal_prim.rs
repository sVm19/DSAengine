...
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_mst_kruskal_prim(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_mst_kruskal_prim(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'edges' as [(u, v, weight)] and 'num_nodes'.".to_string(),
    })?;

    let result = {
        let mut edges = req.edges.clone();
        match MstKruskalPrim::kruskal(req.num_nodes, &mut edges) {
            Some((mst_edges, total_weight)) => {
                json!({ "mst_edges": mst_edges, "total_weight": total_weight, "edge_count": mst_edges.len() })
            }
            None => json!({ "error": "No MST found (graph may be disconnected)" }),
        }
    };

    let solver = MstKruskalPrim;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["kruskal"],
    }))

...
impl Visualizer {
    ...
    pub fn adjacency_list(adj: &[Vec<usize>]) -> String {
        ...
        AgentLogger::log(
            AgentFeedback::Info,
            format!("Rendered adjacency list for {} node(s).", adj.len()),
        );
        out
    }
...
pub struct GraphVisualizerRequest {
    pub adj: Vec<Vec<usize>>,
}

async fn handle_visualizer(payload: Value) -> DsaResult<ResultBox> {
    let req: GraphVisualizerRequest =
        serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
            message: format!("Invalid GraphVisualizerRequest: {e}"),
            hint: "Provide 'adj' as an adjacency list: array of neighbor arrays.".to_string(),
        })?;

    for (node, neighbours) in req.adj.iter().enumerate() {
        for &neighbor in neighbours {
            if neighbor >= req.adj.len() {
                return Err(DsaError::IndexOutOfBounds {
                    index: neighbor,
                    bounds: req.adj.len(),
                    context: format!("neighbor referenced from node {node}"),
                });
            }
        }
    }
    ...
    Ok(ResultBox::success(json!({
        "adjacency_list": Visualizer::adjacency_list(&req.adj),
        "edge_table": Visualizer::edge_table(&req.adj),
        "degree_stats": Visualizer::degree_stats(&req.adj)
    }))
    ...
    .with_description("Graph visualization generated."))
}

...
impl WordLadder {
    ...
    pub fn solve(begin: &str, end: &str, word_list: &[&str]) -> usize {
        ...
        if !word_set.contains(end) {
            AgentLogger::log(
                ...
                format!("End word \"{end}\" not in word list."),
            );
            return 0;
        }
        ...
        AgentLogger::log(
            AgentFeedback::Info,
            format!(
                "Word-ladder BFS: \"{begin}\" → \"{end}\"; {} word(s) in dict.",
                word_list.len()
            ),
        );
    ...
    pub fn find_all_ladders(begin: &str, end: &str, word_list: &[&str]) -> Vec<Vec<String>> {
        ...
        AgentLogger::log(
            AgentFeedback::Success,
            format!(
                "Found {} shortest ladder(s) of length {}.",
                results.len(),
...
#[macros::mcp_tool(
    name = "graphs.word_ladder",
    description = "Use this for solving word ladder problems. Trigger Keywords: graph, word_ladder, shortest path, traversal. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_word_ladder(payload).await {
...
async fn handle_word_ladder(payload: Value) -> DsaResult<ResultBox> {
    let req: WordLadderRequest =
        serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
            message: format!("Invalid WordLadderRequest: {e}"),
            hint: "Provide 'begin', 'end', and 'word_list'.".to_string(),
        })?;

    let words: Vec<&str> = req.word_list.iter().map(String::as_str).collect();
    let length = WordLadder::solve(&req.begin, &req.end, &words);
    let ladders = if req.all_paths.unwrap_or(false) {
        Some(WordLadder::find_all_ladders(&req.begin, &req.end, &words))
    } else {
        None
    };
    ...
    .with_description("Word-ladder BFS completed."))
}

    ...
    pub fn solve(grid: &mut Vec<Vec<u8>>) -> i32 {
        ...
        if fresh == 0 {
            AgentLogger::log(
                ...
                "No fresh oranges; answer is 0 minutes.",
            );
            return 0;
        }
...
async fn handle_rotting_oranges(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'grid' (2D array: 0=empty, 1=fresh, 2=rotten).".to_string(),
    })?;

    let result = {
        let mut grid = req.grid.clone();
        let minutes = RottingOranges::solve(&mut grid);
        json!({ "minutes": minutes, "all_rotted": minutes >= 0, "final_grid": grid })
    };
    ...
    .with_description("Rotting oranges simulation completed."))
}

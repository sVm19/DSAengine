    ...
    pub fn components(adj: &[Vec<usize>]) -> Vec<Vec<usize>> {
        ...
        for start in 0..n {
            ...
            AgentLogger::log(
                AgentFeedback::Step,
                format!(
                    "Component #{}: {} node(s), starting from {start}.",
                    components.len() + 1,
                    component.len()
                ),
            );
...
async fn handle_dfs_generator(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'adj' (adjacency list). Optional 'mode': 'traverse' | 'components'."
            .to_string(),
    })?;

    let result = match req.mode.as_deref().unwrap_or("") {
        "components" => {
            let comps = DfsGenerator::components(&req.adj);
            json!({ "mode": "components", "count": comps.len(), "components": comps })
        }
        _ => {
            let (order, has_cycle) = DfsGenerator::solve(&req.adj);
            json!({ "mode": "traverse", "dfs_order": order, "has_cycle": has_cycle })
        }
    };

    let solver = DfsGenerator;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["traverse", "components"],
    }))
    ...
    .with_description("DFS completed."))
}

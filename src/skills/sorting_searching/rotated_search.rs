...
impl RotatedSearch {
    ...
    pub fn solve(nums: &[i32], target: i32) -> Option<usize> {
        ...
        AgentLogger::log(
            AgentFeedback::Info,
            format!(
                "Rotated array search for target {target} in array of length {}.",
                nums.len()
            ),
        );
        ...
        while l <= r {
            ...
            AgentLogger::log(
                AgentFeedback::Step,
                format!(
                    "Checking interval [{l}, {r}]; mid={mid} (val={}).",
                    nums[mid]
                ),
            );
            ...
            if nums[mid] == target {
                AgentLogger::log(
                    ...
                    format!("Target {target} found at index {mid}."),
                );
                return Some(mid);
            }
        ...
        AgentLogger::log(
            ...
            format!("Target {target} not found in rotated array."),
        );
        None
    }
    ...
    pub fn find_min(nums: &[i32]) -> usize {
        ...
        AgentLogger::log(
            AgentFeedback::Success,
            format!("Rotated array minimum found at index {l}."),
        );
        l
    }
}
...
#[macros::mcp_tool(
    name = "sorting_searching.rotated_search",
    description = "Use this for solving rotated search problems. Trigger Keywords: sorting, searching, rotated_search. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]

...
impl Visualizer {
    ...
    pub fn view_interval<T: std::fmt::Display>(arr: &[T], left: usize, right: usize) -> String {
        ...
        AgentLogger::log(
            AgentFeedback::Step,
            format!(
                "Viewing interval [{safe_left}, {safe_right}] within bounds (len={}).",
                arr.len()
            ),
        );
    ...
    pub fn display_histogram(counts: &[usize], offset: i32) -> String {
        ...
        for (i, &count) in counts.iter().enumerate() {
            ...
            let bars = if max_val > 0 {
                (count * max_chars) / max_val
            } else {
                0
            };
...
#[macros::mcp_tool(
    name = "sorting_searching.visualizer",

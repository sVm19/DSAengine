...
impl MergeSort {
    ...
    pub fn solve<T: Ord + Clone>(arr: &mut [T]) {
        ...
        while width < n {
            AgentLogger::log(
                AgentFeedback::Step,
                format!("Merging subarrays of width {width}."),
            );
...
async fn handle_merge_sort(payload: Value) -> DsaResult<ResultBox> {
    ...
    .with_description("Merge sort completed."))
}

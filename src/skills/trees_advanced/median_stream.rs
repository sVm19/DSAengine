...
impl MedianFinder {
    pub fn new() -> Self {
        AgentLogger::log(AgentFeedback::Info, "Initialized new Two-Heap Median Streamer.");
        Self {
    ...
    pub fn add_num(&mut self, num: i32) {
        ...
        AgentLogger::log(AgentFeedback::Step, format!("Added {num}. Structural parity balanced."));
    }

    pub fn find_median(&self) -> f64 {
        if self.low_max_heap.len() > self.high_min_heap.len() {
            let med = *self.low_max_heap.peek().unwrap() as f64;
            AgentLogger::log(AgentFeedback::Step, format!("Median resolved via distinct middle: {med}."));
            med
        } else {
            let low = *self.low_max_heap.peek().unwrap() as f64;
            let Reverse(high) = *self.high_min_heap.peek().unwrap();
            let med = (low + high as f64) / 2.0;
            AgentLogger::log(AgentFeedback::Step, format!("Median resolved via dual aggregate: {med}."));
            med
        }
    }
}
...
#[macros::mcp_tool(name = "median_stream", description = "Use this for solving median stream problems. Trigger Keywords: median_stream, median stream, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results.")]

...
impl BinaryIndexedTree {
    pub fn new(size: usize) -> Self {
        AgentLogger::log(
            AgentFeedback::Info,
            format!("Initialized Fenwick Tree with capacity {size}."),
        );
        Self {
            tree: vec![0; size + 1],
        }
    ...
    pub fn add(&mut self, mut i: usize, delta: i32) {
        ...
        AgentLogger::log(
            AgentFeedback::Step,
            format!("Fenwick: Adding {delta} at index {i}."),
        );
    ...
    pub fn range_query(&self, l: usize, r: usize) -> i32 {
        ...
        AgentLogger::log(
            AgentFeedback::Step,
            format!("Fenwick Range Query [{l}, {r}] = {val}."),
        );
        val
    }
}
...
#[macros::mcp_tool(
    name = "fenwick_tree",
    description = "Use this for solving fenwick tree problems. Trigger Keywords: fenwick_tree, fenwick tree, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_fenwick_tree(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_fenwick_tree(payload: Value) -> DsaResult<ResultBox> {
    #[derive(Debug, serde::Deserialize, schemars::JsonSchema)]
    struct Request {
        size: usize,
        updates: Option<Vec<(usize, i32)>>,
        queries: Option<Vec<(usize, Option<usize>)>>,
    }

    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        message: format!("Invalid request: {e}"),
        hint: "Provide 'size'. Optional 'updates' as [(index, delta)], 'queries' as [(index, end_index?)].".to_string(),
    })?;

    let result = {
        let mut tree = BinaryIndexedTree::new(req.size);
        ...
        if let Some(queries) = &req.queries {
            for &(l, r) in queries {
                let s = if let Some(r) = r {
                    tree.range_query(l, r)
                } else {
                    tree.query(l)
                };
                results.push(json!({"l":l,"r":r,"sum":s}));
            }
        }
        json!({ "size": req.size, "query_results": results })
    };

    let solver = FenwickTree;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["build", "add", "query"],
    }))
    ...
    .with_description("FenwickTree completed."))
}

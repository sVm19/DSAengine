...
impl TopKElements {
    pub fn solve(arr: &[i32], k: usize) -> Vec<i32> {
        ...
        AgentLogger::log(
            AgentFeedback::Info,
            format!(
                "Filtering Top {k} elements from {}-element stream.",
                arr.len()
            ),
        );
        ...
        for &val in arr {
            ...
            if min_heap.len() > k {
                ...
                AgentLogger::log(
                    AgentFeedback::Step,
                    format!("Added {val}. Heap exceeded {k}. Evicting global minimum {dropped}."),
                );
            }
        }
        ...
        AgentLogger::log(
            ...
            "Top K elements dynamically extracted.",
        );
...
#[macros::mcp_tool(
    name = "trees_advanced.top_k_elements",
    description = "Use this for solving top k elements problems. Trigger Keywords: top_k_elements, top k elements, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]

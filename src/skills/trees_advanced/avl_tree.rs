...
impl AvlArena {
    ...
    pub fn height(&self, node_idx: usize) -> i32 {
        if node_idx == usize::MAX { 0 } else { self.nodes[node_idx].height }
    }
    ...
    pub fn rotate_right(&mut self, y: usize) -> usize {
        ...
        AgentLogger::log(AgentFeedback::Step, format!("Executed LL Right Rotation around node {y}."));
        x
    }
    ...
    pub fn rotate_left(&mut self, x: usize) -> usize {
        ...
        AgentLogger::log(AgentFeedback::Step, format!("Executed RR Left Rotation around node {x}."));
        y
    }
}
...
#[macros::mcp_tool(name = "avl_tree", description = "Use this for solving avl tree problems. Trigger Keywords: avl_tree, avl tree, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results.")]

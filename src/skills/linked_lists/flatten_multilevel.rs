...
#[macros::mcp_tool(
    name = "flatten_multilevel",
    description = "Use this for solving flatten multilevel problems. Trigger Keywords: flatten_multilevel, flatten multilevel, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_flatten_multilevel(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_flatten_multilevel(payload: Value) -> DsaResult<ResultBox> {
    #[derive(Debug, serde::Deserialize, schemars::JsonSchema)]
    struct Request {
        nodes: Vec<(usize, usize, usize, i32)>,
        head: usize,
    }

    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        message: format!("Invalid request: {e}"),
        hint: "Provide 'nodes' as [(next,prev,child,val)] and 'head'.".to_string(),
    })?;

    let result = {
        let mut nodes_c = req.nodes.clone();
        let new_head = FlattenMultilevel::solve(&mut nodes_c, req.head);
        json!({ "head": new_head })
    };

    let solver = FlattenMultilevel;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["flatten"],
    }))
    ...
    .with_description("FlattenMultilevel completed."))
}

    ...
    pub fn solve(a: &[u8], b: &[u8]) -> Vec<u8> {
        ...
        if carry > 0 {
            ...
            AgentLogger::log(
                AgentFeedback::Step,
                format!(
                    "Final carry={carry} appended at position {}.",
                    result.len() - 1
                ),
            );
        }
        ...
        AgentLogger::log(
            ...
            format!(
                "Result: {} digit(s), MSD={}.",
                result.len(),
                result.last().copied().unwrap_or(0)
            ),
        );
        result
    }
...
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_add_two_numbers(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_add_two_numbers(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'a' and 'b' as arrays of digits (least-significant first).".to_string(),
    })?;

    let result = {
        let result = AddTwoNumbers::solve(&req.a, &req.b);
        let num = AddTwoNumbers::to_number(&result);
        json!({ "sum_digits": result, "sum_value": num.to_string() })
    };

    let solver = AddTwoNumbers;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["add", "to_number"],
    }))
    ...
    .with_description("AddTwoNumbers completed."))
}

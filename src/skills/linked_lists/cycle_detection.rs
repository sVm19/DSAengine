...
impl CycleDetection {
    ...
    pub fn has_cycle(nodes: &[(usize, usize)], head: usize) -> bool {
        ...
        loop {
            ...
            if slow == fast {
                AgentLogger::log(AgentFeedback::Warning, format!("Cycle detected: meeting point={slow}."));
                return true;
            }
        }
    }
...
#[macros::mcp_tool(name = "cycle_detection", description = "Use this for solving cycle detection problems. Trigger Keywords: cycle_detection, cycle detection, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results.")]

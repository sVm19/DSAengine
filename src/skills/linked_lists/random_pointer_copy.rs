...
#[macros::mcp_tool(
    name = "random_pointer_copy",
    description = "Use this for solving random pointer copy problems. Trigger Keywords: random_pointer_copy, random pointer copy, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_random_pointer_copy(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_random_pointer_copy(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'nodes' as [(next,random,val)] and 'head'.".to_string(),
    })?;

    let result = {
        let mut nc = req.nodes.clone();
        let h = RandomPointerCopy::solve(&mut nc, req.head);
        json!({ "head": h })
    };

    let solver = RandomPointerCopy;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["copy"],
    }))
    ...
    .with_description("RandomPointerCopy completed."))
}

...
impl Serialization {
    ...
    pub fn solve_serialize(nodes: &[(usize, usize, i32)], root: usize) -> String {
        let null = usize::MAX;
        if root == null { return String::new(); }

        let mut out = String::new();
        let mut queue = VecDeque::new();
        queue.push_back(root);

        AgentLogger::log(AgentFeedback::Info, "Serializing binary tree level-by-level.");
...
#[macros::mcp_tool(name = "serialization", description = "Use this for solving serialization problems. Trigger Keywords: serialization, serialization, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results.")]

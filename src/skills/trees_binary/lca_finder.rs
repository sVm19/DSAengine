...
impl LcaFinder {
    pub fn solve(nodes: &[(usize, usize, i32)], root: usize, p: usize, q: usize) -> usize {
        ...
        AgentLogger::log(
            AgentFeedback::Info,
            format!("Searching for LCA iteractively for nodes {p} and {q}."),
        );
        ...
        AgentLogger::log(
            ...
            format!("LCA resolved to index {lca} (value={}).", nodes[lca].2),
        );
        lca
    }
}
...
async fn handle_lca_finder(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'nodes', 'root', 'p', 'q'.".to_string(),
    })?;

    let result = {
        let lca = LcaFinder::solve(&req.nodes, req.root, req.p, req.q);
        json!({ "lca_node": lca })
    };

    let solver = LcaFinder;
    ...
    .with_description("LcaFinder completed."))
}

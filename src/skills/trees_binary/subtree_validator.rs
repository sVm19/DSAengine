...
impl SubtreeValidator {
    pub fn solve(
        m_nodes: &[(usize, usize, i32)],
        m_root: usize,
        s_nodes: &[(usize, usize, i32)],
        s_root: usize,
    ) -> bool {
        let null = usize::MAX;
        if s_root == null {
        ...
        AgentLogger::log(
            AgentFeedback::Info,
            "Commencing iterative subtree validation scan.",
        );
        ...
        while let Some(curr) = queue.pop_front() {
            if Self::is_same(m_nodes, curr, s_nodes, s_root) {
                AgentLogger::log(
                    ...
                    format!("Subtree match anchored at node index {curr}."),
                );
                return true;
            }
            if m_nodes[curr].0 != null {
                queue.push_back(m_nodes[curr].0);
            }
            if m_nodes[curr].1 != null {
                queue.push_back(m_nodes[curr].1);
            }
        }

        AgentLogger::log(
            AgentFeedback::Warning,
            "Subtree validation failed. No isomorphic match found.",
        );
        false
    }
...
#[macros::mcp_tool(
    name = "subtree_validator",
    description = "Use this for solving subtree validator problems. Trigger Keywords: subtree_validator, subtree validator, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_subtree_validator(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_subtree_validator(payload: Value) -> DsaResult<ResultBox> {
    #[derive(Debug, serde::Deserialize, schemars::JsonSchema)]
    struct Request {
        main_nodes: Vec<(usize, usize, i32)>,
        main_root: usize,
        sub_nodes: Vec<(usize, usize, i32)>,
        sub_root: usize,
    }

    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        message: format!("Invalid request: {e}"),
        hint: "Provide main and sub tree nodes+root.".to_string(),
    })?;

    let result = {
        let is =
            SubtreeValidator::solve(&req.main_nodes, req.main_root, &req.sub_nodes, req.sub_root);
        json!({ "is_subtree": is })
    };

    let solver = SubtreeValidator;
    ...
    .with_description("SubtreeValidator completed."))
}

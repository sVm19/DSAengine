...
impl Visualizer {
    pub fn to_string(nodes: &[(usize, usize, i32)], root: usize) -> String {
        let null = usize::MAX;
        if root == null { return String::from("[Empty Tree]"); }
        ...
        AgentLogger::log(AgentFeedback::Info, "Rendering discrete tree topological array limits.");
        ...
        while !queue.is_empty() {
            ...
            for _ in 0..limit {
                ...
                if nodes[curr].0 != null { queue.push_back(nodes[curr].0); }
                if nodes[curr].1 != null { queue.push_back(nodes[curr].1); }
            }
            out.push('\n');
            level += 1;
        }

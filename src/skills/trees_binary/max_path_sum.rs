...
impl MaxPathSum {
    pub fn solve(nodes: &[(usize, usize, i32)], root: usize) -> i32 {
        ...
        AgentLogger::log(AgentFeedback::Info, "Traversing tree iteratively to find global maximum path sum.");
        ...
        while curr != null || !stack.is_empty() {
            if curr != null {
                ...
                if right != null && last_visited != right {
                    ...
                    let left_branch = if left != null { max_branch[left].max(0) } else { 0 };
                    let right_branch = if right != null { max_branch[right].max(0) } else { 0 };
        ...
        AgentLogger::log(AgentFeedback::Success, format!("Global max path sum is {max_sum}."));
        max_sum
    }
}
...
#[macros::mcp_tool(name = "max_path_sum", description = "Use this for solving max path sum problems. Trigger Keywords: max_path_sum, max path sum, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results.")]

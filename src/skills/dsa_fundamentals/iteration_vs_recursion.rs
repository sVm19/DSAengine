...
#[macros::mcp_tool(
    name = "dsa_fundamentals.iteration_vs_recursion",
    description = "Use this for solving iteration vs recursion problems. Trigger Keywords: iteration_vs_recursion, iteration vs recursion, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_iteration_vs_recursion(payload).await {
...
pub struct IterationVsRecursionRequest {
    pub n: u64,
    pub recursion_depth: Option<usize>,
}

async fn handle_iteration_vs_recursion(payload: Value) -> DsaResult<ResultBox> {
    let req: IterationVsRecursionRequest =
        serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
            message: format!("Invalid IterationVsRecursionRequest: {e}"),
            hint: "Provide 'n' and optional 'recursion_depth'.".to_string(),
        })?;

    let depth = req.recursion_depth.unwrap_or(req.n as usize);
    IterationVsRecursion::warn_stack_depth(depth);
    let iterative_factorial =
        (1..=req.n).fold(1u128, |acc, value| acc.saturating_mul(value as u128));

    let solver = IterationVsRecursion;
    Ok(ResultBox::success(json!({
        "n": req.n,
        "iterative_factorial_saturating": iterative_factorial,
        "estimated_recursion_depth": depth,
        "stack_overflow_risk": depth > 1000,
        "recommendation": if depth > 1000 { "Prefer iteration or an explicit stack." } else { "Either iteration or recursion is viable; iteration keeps stack usage constant." }
    }))
    ...
    .with_description("Iteration-vs-recursion analysis completed."))
}

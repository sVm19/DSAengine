...
pub struct RainwaterTrappingRequest {
    pub heights: Vec<i32>,
}

#[macros::mcp_tool(
    name = "arrays_strings.rainwater_trapping",
    description = "Use this for solving rainwater trapping problems. Trigger Keywords: rainwater_trapping, rainwater trapping, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_rainwater_trapping(payload).await {
...
async fn handle_rainwater_trapping(payload: Value) -> DsaResult<ResultBox> {
    let req: RainwaterTrappingRequest =
        serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
            message: format!("Invalid RainwaterTrappingRequest: {e}"),
            hint: "Provide 'heights' array.".to_string(),
        })?;

    let trapped = RainwaterTrapping::solve(&req.heights);
    let solver = RainwaterTrapping;
    let complexity = json!({
        "name": solver.name(),
    ...
    .with_description("Rainwater trapping computation completed."))
}

...
pub struct ThreeSumSolverRequest {
    pub nums: Vec<i32>,
}

#[macros::mcp_tool(
    name = "arrays_strings.three_sum_solver",
    description = "Use this for solving three sum solver problems. Trigger Keywords: three_sum_solver, three sum solver, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_three_sum_solver(payload).await {
...
async fn handle_three_sum_solver(payload: Value) -> DsaResult<ResultBox> {
    let req: ThreeSumSolverRequest =
        serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
            message: format!("Invalid ThreeSumSolverRequest: {e}"),
            hint: "Provide 'nums'.".to_string(),
        })?;
    let mut nums = req.nums;
    let triplets = ThreeSumSolver::solve(&mut nums);
    ...
    .with_description("Three-sum triplet discovery completed."))
}

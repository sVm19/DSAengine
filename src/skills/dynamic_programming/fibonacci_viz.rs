...
impl FibonacciViz {
    ...
    pub fn fast(n: u64) -> u64 {
        ...
        while exp > 0 {
            if exp & 1 == 1 {
                ...
                AgentLogger::log(
                    AgentFeedback::Step,
                    format!(
                        "Multiplied result by base at exp bit; remaining exp={}.",
                        exp >> 1
                    ),
                );
            }
        ...
        AgentLogger::log(
            AgentFeedback::Success,
            format!("Matrix-exp F({n}) = {}.", result[1]),
        );
        result[1]
    }
    ...
    fn mat_mul(a: [u64; 4], b: [u64; 4]) -> [u64; 4] {
        [
            a[0].saturating_mul(b[0])
                .saturating_add(a[1].saturating_mul(b[2])),
            a[0].saturating_mul(b[1])
                .saturating_add(a[1].saturating_mul(b[3])),
            a[2].saturating_mul(b[0])
                .saturating_add(a[3].saturating_mul(b[2])),
            a[2].saturating_mul(b[1])
                .saturating_add(a[3].saturating_mul(b[3])),
        ]
    }
}
...
#[macros::mcp_tool(
    name = "dynamic_programming.fibonacci_viz",
    description = "Use this for solving fibonacci viz problems. Trigger Keywords: fibonacci_viz, fibonacci viz, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_fibonacci_viz(payload).await {
...
pub struct FibonacciVizRequest {
    pub n: u64,
...
async fn handle_fibonacci_viz(payload: Value) -> DsaResult<ResultBox> {
    let req: FibonacciVizRequest =
        serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
            message: format!("Invalid FibonacciVizRequest: {e}"),
            hint: "Provide 'n', optional 'mode' ('iterative', 'fast', or 'sequence'), and optional 'count'.".to_string(),
        })?;

    let mode = req.mode.unwrap_or_else(|| "iterative".to_string());
    let result = match mode.as_str() {
        "fast" => json!({ "mode": "fast", "value": FibonacciViz::fast(req.n) }),
        "sequence" => {
            let count = req.count.unwrap_or(req.n as usize + 1);
            json!({ "mode": "sequence", "sequence": FibonacciViz::sequence(count) })
        }
        _ => json!({ "mode": "iterative", "value": FibonacciViz::solve(req.n) }),
    };
    let solver = FibonacciViz;
    ...
    .with_description("Fibonacci computation completed."))
}

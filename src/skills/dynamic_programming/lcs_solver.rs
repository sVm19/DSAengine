...
impl LcsSolver {
    ...
    pub fn solve(s: &str, t: &str) -> usize {
        ...
        let result = prev[n];
        AgentLogger::log(AgentFeedback::Success, format!("LCS length = {result}."));
        result
    }
...
#[macros::mcp_tool(
    name = "lcs_solver",
    description = "Use this for solving lcs solver problems. Trigger Keywords: lcs_solver, lcs solver, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_lcs_solver(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_lcs_solver(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 's' and 't' strings. Optional 'mode': 'length' | 'sequence'.".to_string(),
    })?;

    let result = match req.mode.as_deref().unwrap_or("") {
        "sequence" => {
            let seq = LcsSolver::reconstruct(&req.s, &req.t);
            let len = seq.len();
            json!({ "mode": "sequence", "length": len, "subsequence": seq })
        }
        _ => {
            let len = LcsSolver::solve(&req.s, &req.t);
            json!({ "mode": "length", "length": len })
        }
    };

    let solver = LcsSolver;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["length", "sequence"],
    }))

        ...
        for i in 1..=m {
            ...
            for j in 1..=n {
                curr[j] = if p[i - 1] == b'*' {
                    ...
                    let extends =
                        i >= 2 && (p[i - 2] == t[j - 1] || p[i - 2] == b'.') && curr[j - 1];
                    zero_use || extends
                } else {
...
#[macros::mcp_tool(
    name = "regex_matching",
    description = "Use this for solving regex matching problems. Trigger Keywords: regex_matching, regex matching, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_regex_matching(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_regex_matching(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'pattern' and 'text'. Optional 'mode': 'match' | 'full'.".to_string(),
    })?;

    let result = match req.mode.as_deref().unwrap_or("") {
        "full" => {
            let r = RegexMatching::solve_full(&req.pattern, &req.text);
            json!({ "mode": "full", "matched": r })
        }
        _ => {
            let r = RegexMatching::solve(&req.pattern, &req.text);
            json!({ "mode": "match", "matched": r })
        }
    };

    let solver = RegexMatching;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["match", "full"],
    }))
    ...
    .with_description("Regex matching completed."))
}

    ...
    pub fn solve(pattern: &str, text: &str) -> bool {
        ...
        while ti < n {
            if pi < m && (p[pi] == t[ti] || p[pi] == b'?') {
                AgentLogger::log(
                    AgentFeedback::Step,
                    format!("Matched p[{pi}]='{}' with t[{ti}]='{}'.", p[pi] as char, t[ti] as char),
                );
                pi += 1;
                ti += 1;
            } else if pi < m && p[pi] == b'*' {
...
#[macros::mcp_tool(name = "wildcard_matching", description = "Use this for solving wildcard matching problems. Trigger Keywords: wildcard_matching, wildcard matching, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results.")]

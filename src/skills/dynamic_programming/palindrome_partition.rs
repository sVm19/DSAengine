...
impl PalindromePartition {
    ...
    pub fn partition(s: &str) -> Vec<String> {
        ...
        AgentLogger::log(
            AgentFeedback::Success,
            format!(
                "Partitioned into {} palindromic piece(s): {:?}.",
                parts.len(),
                parts
            ),
        );
...
#[macros::mcp_tool(
    ...
    description = "Use this for solving palindrome partition problems. Trigger Keywords: palindrome_partition, palindrome partition, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_palindrome_partition(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_palindrome_partition(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 's' (string). Optional 'mode': 'min_cuts' | 'partition'.".to_string(),
    })?;

    let result = match req.mode.as_deref().unwrap_or("") {
        "partition" => {
            let parts = PalindromePartition::partition(&req.s);
            json!({ "mode": "partition", "parts": parts })
        }
        _ => {
            let cuts = PalindromePartition::solve(&req.s);
            json!({ "mode": "min_cuts", "min_cuts": cuts })
        }
    };

    let solver = PalindromePartition;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["min_cuts", "partition"],
    }))
    ...
    .with_description("Palindrome partition completed."))
}

...
impl EditDistance {
    ...
    pub fn solve_with_trace(src: &str, dst: &str) -> (usize, Vec<String>) {
        ...
        AgentLogger::log(
            AgentFeedback::Success,
            format!(
                "Traceback produced {} operation(s) with total cost {}.",
                ops.len(),
                dp[m][n]
            ),
        );
...
#[macros::mcp_tool(
    name = "edit_distance",
    description = "Use this for solving edit distance problems. Trigger Keywords: edit_distance, edit distance, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_edit_distance(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_edit_distance(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'source' and 'target' strings. Optional 'mode': 'distance' | 'operations'."
            .to_string(),
    })?;

    let result = match req.mode.as_deref().unwrap_or("") {
        "operations" => {
            let (dist, ops) = EditDistance::solve_with_trace(&req.source, &req.target);
            json!({ "mode": "operations", "distance": dist, "operations": ops })
        }
        _ => {
            let dist = EditDistance::solve(&req.source, &req.target);
            json!({ "mode": "distance", "distance": dist })
        }
    };

    let solver = EditDistance;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["distance", "operations"],
    }))
    ...
    .with_description("Edit distance computed."))
}

...
#[macros::mcp_tool(
    ...
    description = "Use this for solving pattern matcher problems. Trigger Keywords: pattern_matcher, pattern matcher, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_pattern_matcher(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_pattern_matcher(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'pattern' and 'text'. Optional 'mode': 'match' | 'contains'.".to_string(),
    })?;

    let result = match req.mode.as_deref().unwrap_or("") {
        "contains" => {
            let found = PatternMatcher::contains(&req.pattern, &req.text);
            json!({ "mode": "contains", "found": found })
        }
        _ => {
            let matched = PatternMatcher::solve(&req.pattern, &req.text);
            json!({ "mode": "match", "matched": matched })
        }
    };

    let solver = PatternMatcher;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["match", "contains"],
    }))
    ...
    .with_description("Pattern matching completed."))
}

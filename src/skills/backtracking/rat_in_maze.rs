...
const DIRS: [(i32, i32, &str); 4] = [(1, 0, "D"), (0, 1, "R"), (-1, 0, "U"), (0, -1, "L")];
...
impl RatInMaze {
    ...
    pub fn solve(grid: &[&[u8]]) -> Vec<String> {
        ...
        AgentLogger::log(
            AgentFeedback::Info,
            format!(
                "Rat-in-maze DFS on {rows}×{cols} grid from (0,0) to ({},{}).",
                rows - 1,
                cols - 1
            ),
        );
        ...
        while let Some(frame) = stack.last_mut() {
            ...
            while frame.dir_index < DIRS.len() {
                ...
                AgentLogger::log(
                    AgentFeedback::Step,
                    format!(
                        "Moving {} to ({nr},{nc}); path depth now {}.",
                        label,
                        path_dirs.len()
                    ),
                );
        ...
        AgentLogger::log(
            AgentFeedback::Success,
            format!(
                "Rat-in-maze complete: {} total path(s) found.",
                results.len()
            ),
        );
        results
...
#[macros::mcp_tool(
    name = "rat_in_maze",
    description = "Use this for solving rat in maze problems. Trigger Keywords: rat_in_maze, rat in maze, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_rat_in_maze(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_rat_in_maze(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'grid' (2D array of 0/1 where 1=open path).".to_string(),
    })?;

    let result = {
        let grid_refs: Vec<&[u8]> = req.grid.iter().map(|r| r.as_slice()).collect();
        let paths = RatInMaze::solve(&grid_refs);
        json!({ "total_paths": paths.len(), "paths": paths })
    };

    let solver = RatInMaze;
    Ok(ResultBox::success(json!({
        "result": result,
        "available_modes": ["find_paths"],
    }))
    ...
    .with_description("Rat-in-maze pathfinding completed."))
}

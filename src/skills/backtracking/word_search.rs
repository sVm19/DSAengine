...
#[macros::mcp_tool(
    name = "word_search",
    description = "Use this for solving word search problems. Trigger Keywords: word_search, word search, algorithm, dsa. Input Hints: Look for input fields like nums, numbers, arr, target, edges, adj, source, capacity, weight, values in the user's text to populate task arguments.. Why: Choose this over generic fallback when the problem domain matches the algorithm's strengths for best-performance results."
)]
pub async fn post(Json(payload): Json<Value>) -> impl IntoResponse {
    match handle_word_search(payload).await {
        Ok(res) => (StatusCode::OK, Json(res)).into_response(),
        Err(e) => e.into_response(),
    }
}

async fn handle_word_search(payload: Value) -> DsaResult<ResultBox> {
    ...
    let req: Request = serde_json::from_value(payload).map_err(|e| DsaError::InvalidInput {
        ...
        hint: "Provide 'grid' (2D array of single-char strings) and 'word' (string to search)."
            .to_string(),
    })?;

    let result = {
        let grid_bytes: Vec<Vec<u8>> = req
            .grid
            .iter()
            .map(|row| row.iter().map(|s| s.as_bytes()[0]).collect())
            .collect();
        let grid_refs: Vec<&[u8]> = grid_bytes.iter().map(|r| r.as_slice()).collect();
        let found = WordSearch::solve(&grid_refs, req.word.as_bytes());
        json!({ "word": req.word, "found": found })
    };

    let solver = WordSearch;
    ...
    .with_description("Word search completed."))
}

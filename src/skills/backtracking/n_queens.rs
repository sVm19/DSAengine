    ...
    pub fn solve(n: usize) -> Vec<Vec<usize>> {
        ...
        loop {
            if col_cursor[row] == n {
                ...
                AgentLogger::log(
                    AgentFeedback::Step,
                    format!("Backtracked to row {row}, will try column {} next.", col_cursor[row]),
                );
                continue;
            }
        ...
        AgentLogger::log(
            AgentFeedback::Success,
            format!("N-Queens({n}) found {} distinct solution(s).", results.len()),
        );
        results
    }

...
impl SudokuSolver {
    ...
    pub fn solve(board: &mut [u8; 81]) -> bool {
        ...
        AgentLogger::log(
            AgentFeedback::Info,
            format!("Sudoku: {} empty cells detected; starting bitmask-constrained search.", empty.len()),
        );
    ...
    fn candidates(cell: usize, row_mask: &[u16; 9], col_mask: &[u16; 9], box_mask: &[u16; 9]) -> u16 {
        let row = cell / 9;
        let col = cell % 9;
        let bx = (row / 3) * 3 + col / 3;
